...
with open(config_file_path, "r",encoding="utf-8") as file:
    ...
    for line in file:
        ...
        if line.startswith("CATEGORY"):
            ...
            threshold_km1_name = "Zone de "+str(threshold_km1)+" kilom̬tres"
            ...
            threshold_km2_name = "Zone de " + str(threshold_km2) + " kilom̬tres"
...
for _, row in df_points.iterrows():
    ...
    if shape == "circle":
        ...
        shape_html = f'<i style="display:inline-block; width:12px; height:12px; background-color:{color};"></i>'  # Par d̩faut
...
for feature in belgium_geojson['features']:
    if feature['properties']['name'] in highlight_area_vlaanderen:  # Modifier la cl̩ selon votre GeoJSON
        folium.GeoJson(
            feature,
...
for cluster_label in set(data['cluster']):
    ...
    if len(cluster_points) > 1:
        ...
        folium.Circle(
            ...
            fill_color=color_point,
            fill_opacity=0.6,
            popup=f"Point isol̩ : {single_point['name']}"
        ).add_to(grouped_points_layer)
